...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <EditorialHero
        variant="article"
        eyebrow="PromptCraft Guide"
        title={guide.title}
        description={`A practical guide with examples, reusable prompts, and workflow notes for ${guide.tags.slice(0, 3).join(', ')}.`}
      />
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        <nav className="mb-6 text-sm text-gray-500">
          <Link href="/" className="hover:text-gray-900">Home</Link>
          <span className="mx-2">/</span>
          <Link href="/guide" className="hover:text-gray-900">Guides</Link>
        </nav>
